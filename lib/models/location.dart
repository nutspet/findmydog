...
const List<Map<String, Map<String, List>>> locations = [
  {
    "北京市": {
      "北京市": [
        "东城区",
...
List<Map<String, List<Map<String, List>>>> locations2 =
locations.map((e) => e.map((k, v) => MapEntry(k, [v]))).toList();

// 生成picker用的数据格式，仿照微信加入了额外选项，方便设置全部区域。
List<Map<String, List<Map<String, List>>>> convertPickerData(
    List<Map<String, Map<String, List>>> locations,
    {String extra = "全部区域"}) {
  // 全局的加一个
  List<Map<String, Map<String, List>>> all = [
    {
      extra: {
        extra: [extra]
      }
  ...
  return all.map((e) {
    return e.map((k, v) {
      v = v.map((k2, v2) {
        if (k != extra) {
          List e = [extra];
          e.addAll(List.from(v2));
          return MapEntry(k2, e);
        } else {
          return MapEntry(k2, v2);
        }
      });
      //return MapEntry(k, [v]);
      // 初始化 省市最前面放一个
      // print(k);
      if (k != extra) {
        Map<String, List<dynamic>> init = {
          extra: [extra]
        };
        init.addAll(v);
        return MapEntry(k, [init]);
      } else {
        // print("有一个吧");
        return MapEntry(k, [v]);
      }
